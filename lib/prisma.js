...
function withConnectionTimeouts(url) {
  ...
  try {
    ...
    // Pool size. Left to Prisma's default (num_cpus * 2 + 1) unless explicitly
    // configured, because the right value depends on the host: too low is a
    // silent throughput ceiling that looks like "the database is slow", too
    // high exhausts a serverless pooler. Set PRISMA_CONNECTION_LIMIT per
    // environment once pool saturation is being measured.
    if (!parsed.searchParams.has('connection_limit') && process.env.PRISMA_CONNECTION_LIMIT) {
      parsed.searchParams.set('connection_limit', String(process.env.PRISMA_CONNECTION_LIMIT));
    }
