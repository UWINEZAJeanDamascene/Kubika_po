...
class JournalService {
  ...
  static async createEntry(companyId, userId, options) {
    ...
    const createFn = async (sess) => {
      ...
      await bankTransactionService.createFromJournalEntry(saved, {
        ...
        transferFromAccountId: options.transferFromAccountId || null,
        transferToAccountId: options.transferToAccountId || null,
        skipBankTransactions: options.skipBankTransactions === true,
        session: sess
      });
  ...
  static async createInvoicePaymentEntry(companyId, userId, payment) {
    ...
    lines.push(this.createDebitLine(
      cashAccount,
      payment.amount,
      `Receipt - ${payment.customerName || 'Customer'} - ${payment.invoiceNumber}`
    ));
    ...
    return this.createEntry(companyId, userId, {
      ...
      sourceType: 'payment',
      sourceId: payment.invoiceId || null,
  ...
  static async createPurchaseEntry(companyId, userId, purchase, options = {}) {
    const lines = [];
    const total = Number(purchase.total ?? purchase.roundedAmount ?? purchase.totalAmount ?? purchase.grandTotal ?? 0) || 0;
    const vatAmount = Number(purchase.vatAmount ?? purchase.totalTax ?? purchase.taxAmount ?? 0) || 0;
    const subtotal = total > 0 ? Math.max(0, total - vatAmount) : 0;

    if (total <= 0) {
      throw new Error('Purchase journal entry requires a positive total amount');
    }
    ...
    // If VAT is 0, inventory debit alone isn't enough — still need AP credit (always added below).
    // If somehow only VAT (no subtotal), still need a balancing debit: use total as inventory.
    if (lines.length === 0) {
      const invAcct = await this.getMappedAccountCode(companyId, 'purchases', 'inventory', DEFAULT_ACCOUNTS.inventory);
      lines.push(this.createDebitLine(
        invAcct,
    ...
    return this.createEntry(companyId, userId, {
      ...
      sourceData: {
        vatAmount,
        netAmount: subtotal || (total - vatAmount),
        grossAmount: total,
   ...
   * Surplus: DR Inventory, CR Stock Adjustment Gain (P&L revenue)
   * Shortage: DR Stock Adjustment Loss (P&L COGS), CR Inventory
   */
  static async createStockAdjustmentEntry(companyId, userId, adjustment, options = {}) {
    const lines = [];
    ...
    const productName = adjustment.productName || 'Inventory item';
    ...
    if (adjustmentType === 'increase') {
      lines.push(this.createDebitLine(
        inventoryAcct,
        amount,
        `Stock adjustment increase: ${productName}${reason}`
      ));

      lines.push(this.createCreditLine(
        DEFAULT_ACCOUNTS.stockAdjustmentGain || DEFAULT_ACCOUNTS.otherIncome || '4200',
        amount,
        `Stock adjustment gain: ${productName}${reason}`
      ));
    } else {
      lines.push(this.createDebitLine(
        DEFAULT_ACCOUNTS.stockAdjustmentLoss || DEFAULT_ACCOUNTS.stockAdjustment || '5150',
        amount,
        `Stock adjustment loss: ${productName}${reason}`
      ));

      lines.push(this.createCreditLine(
        inventoryAcct,
        amount,
        `Stock adjustment decrease: ${productName}${reason}`
      ));
    }

    return this.createEntry(companyId, userId, {
      date: adjustment.date || new Date(),
      description: `Stock adjustment: ${productName} - ${adjustmentType}${reason}`,
      sourceType: 'stock_adjustment',
      sourceId: adjustment._id,
      sourceReference: adjustment.reference || adjustment.referenceNumber,
      lines,
      isAutoGenerated: true
    }, options);
  }
